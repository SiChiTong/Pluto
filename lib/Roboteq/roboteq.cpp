...
bool Roboteq::IsConnected()
{
    return handle != RQ_INVALID_HANDLE;
}

int Roboteq::Connect(std::string port)
{
    if(IsConnected())
    {
...
int Roboteq::ReadAll(string &str)
{
    int countRcv;
    if(!IsConnected())
        return RQ_ERR_NOT_CONNECTED;
...
int Roboteq::IssueCommand(string commandType, string command, string args, int waitms, string &response, bool isplusminus)
{
    int status;
    string read;
    response = "";
...
int Roboteq::IssueCommand(string commandType, string command, int waitms, string &response, bool isplusminus)
{
    return IssueCommand(commandType, command, "", waitms, response, isplusminus);
}

int Roboteq::SetConfig(int configItem, int index, int value)
{
    string response;
    char command[10];
    char args[50];
...
int Roboteq::SetConfig(int configItem, int value)
{
    return SetConfig(configItem, MISSING_VALUE, value);
}

int Roboteq::SetCommand(int commandItem, int index, int value)
{
    string response;
    char command[10];
    char args[50];
...
int Roboteq::SetCommand(int commandItem, int value)
{
    return SetCommand(commandItem, MISSING_VALUE, value);
}

int Roboteq::SetCommand(int commandItem)
{
    return SetCommand(commandItem, MISSING_VALUE, MISSING_VALUE);
}

int Roboteq::GetConfig(int configItem, int index, int &result)
{
    string response;
    char command[10];
    char args[50];
...
int Roboteq::GetConfig(int configItem, int &result)
{
    return GetConfig(configItem, 0, result);
}

int Roboteq::GetValue(int operatingItem, int index, int &result)
{
    string response;
    char command[10];
    char args[50];
...
int Roboteq::GetValue(int operatingItem, int &result)
{
